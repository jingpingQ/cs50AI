...
knowledge3 = And(
    ...
    Implication(BKnave, AKnight),   # If B is a knight, A must be knave
...
def main():
    ...
    for puzzle_name, knowledge in puzzles:
        print(puzzle_name)
            ...
            # For other puzzles, evaluate the knowledge base as usual
        if len(knowledge.conjuncts) == 0:
                ...
                for symbol in symbols:
                    if model_check(knowledge, symbol):
                        print(f"    {symbol}")
        print()  # Print a blank line between each puzzle's results
