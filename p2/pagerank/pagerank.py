...
def iterate_pagerank(corpus, damping_factor):
    ...
    while True:
        ...
        for page in corpus:
            ...
            linking_pages = {p for p in corpus if page in modified_corpus[p]}
            
            pagerank_sum = sum(old_pagerank[p] / len(modified_corpus[p]) 
                             for p in linking_pages)
            
            # Apply
            pagerank[page] = ((1 - damping_factor) / N) + (damping_factor * pagerank_sum)
        
        if all(abs(pagerank[p] - old_pagerank[p]) < 0.001 
               for p in pagerank):
            break
